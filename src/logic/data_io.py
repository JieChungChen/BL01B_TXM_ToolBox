...
def read_txm_raw(filename, mode):
    ...
    if mode == 'tomo':
        return raw_imgs, metadata, angles, reference
    if mode == 'mosaic':
...
def read_multiple_txrm(filelist):
    ...
    all_thetas = []
    for f in filelist:
        images, metadata, thetas, ref = read_txm_raw(f, mode='tomo')
        if ref is not None:
    ...
    sort_id = np.argsort(thetas)
    return images[sort_id], thetas[sort_id]
